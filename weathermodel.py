...
class GFSModel(NOAAModel):
    ...
    def create_grib_url(self, location, time_index):
        ...
        url = self._base_gfs_grib_url.format(self.name, model_run_str, self.model_degree, hour_str, float(math.floor(location.longitude)), float(math.ceil(location.longitude)), float(math.ceil(location.latitude)), float(math.floor(location.latitude)), date_str)
        return url
