...
class TestBuoyData(TestCase):
    ...
    WAVE_FORECAST_BULLETIN_FILE = os.path.join(os.path.dirname(__file__), 'data', 'multi_1.44097.bull')
    ...
    def test_parse_wave_bulletin(self):
        ...
        wave_forecast_data = block_island_station.parse_wave_forecast_bulletin(raw_wave_bulletin_data)
        self.assertTrue(wave_forecast_data != None)
        self.assertTrue(len(wave_forecast_data) > 0)
